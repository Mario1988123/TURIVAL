...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex h-screen bg-slate-100">
      <aside className={`fixed inset-y-0 left-0 z-50 w-72 bg-gradient-to-b from-slate-900 via-slate-900 to-slate-950 text-white shadow-2xl transition-transform duration-300 ease-out md:relative md:translate-x-0 flex flex-col ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        ...
        <nav className="flex-1 overflow-y-auto py-6 px-4 space-y-1 scrollbar-thin">
          {MENU_ITEMS.map((item) => {
            ...
            return (
              <Link key={item.href} href={item.href} onClick={() => setIsOpen(false)} className="flex items-center gap-3 px-4 py-3 rounded-xl text-slate-300 hover:text-white hover:bg-white/10 transition-all duration-200 group">
        ...
        <div className="border-t border-white/10 p-4 space-y-3">
          ...
          {user?.rol === 'admin' && (
            <Link href="/dashboard/admin" className="flex items-center gap-3 px-4 py-3 rounded-xl bg-blue-500/20 text-blue-300 hover:bg-blue-500/30 transition-colors">
              <Shield className="w-5 h-5" />
              <span className="font-medium text-sm">Panel Admin</span>
            </Link>
          )}
          <button onClick={handleLogout} className="w-full flex items-center gap-3 px-4 py-3 rounded-xl text-slate-400 hover:text-red-400 hover:bg-red-500/10 transition-colors">
