...
export default function PresupuestosLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex h-screen bg-background">
        ...
        <div className="flex flex-col h-full">
          ...
          <nav className="flex-1 overflow-y-auto p-4 space-y-2">
            {MENU_ITEMS.map((item) => {
              ...
              return (
                <Link key={item.href} href={item.href} className="flex items-center gap-3 px-4 py-2 rounded-lg hover:bg-slate-800 transition-colors" onClick={() => setIsOpen(false)}>
      ...
      <button className="md:hidden fixed top-4 left-4 z-40 p-2 bg-slate-900 text-white rounded-lg" onClick={() => setIsOpen(!isOpen)}>
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <header className="bg-white border-b border-slate-200 px-6 py-4 shadow-sm">
          <div className="flex items-center justify-between">
            ...
            <span className="text-sm text-slate-600">{new Date().toLocaleDateString('es-ES', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}</span>
          </div>
        </header>
